...
#   WOO_to_CDH_Oberflaeche.exe  neue Oberfläche (Import + Einstellungen)
...
try {
    ...
    python -m PyInstaller @Gemeinsam --windowed --name WOO_to_CDH_Oberflaeche `
        --add-data "ui;ui" --collect-submodules webview `
        --hidden-import openpyxl oberflaeche.py
    if ($LASTEXITCODE -ne 0) { throw "Build Oberflaeche fehlgeschlagen." }
    python -m PyInstaller @Gemeinsam --windowed --name Lieferadressen adressen_gui.py
...
if ($Deploy) {
    ...
    Write-Host "   Bisherige EXE-Dateien gesichert in $Backup"
    foreach ($e in $Exes) { Copy-Item -Force (Join-Path dist $e) $Target }
